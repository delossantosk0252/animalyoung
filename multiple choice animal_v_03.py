...
class YoungAnimalQuiz:
    def __init__(self, root):
        ...
        self.questions = self.load_questions_from_csv('C:/Users/GGPC/Downloads/quiz/animals_young_only.csv')
    ...
    def load_questions_from_csv(self, csv_file):
        questions = []
        with open(csv_file, 'animals_young_only.csv') as file:
            animals_young_only = list(csv.reader(file, delimiter=","))
            animals_young_only.pop(0)
    ...
    def display_question(self):
        ...
        score_label = tk.Label(self.root, text=f"Score: {self.score}", font=("Helvetica", 14), bg="#F0F4C3")
        ...
        if self.round_count < self.num_rounds:
            ...
            option_frame.grid(row=3, column=0, columnspan=2)
            ...
            for i, option in enumerate(question_data["options"]):
                ...
                option_button.grid(row=i // 2, column=i % 2, padx=10, pady=5)  # Two buttons per row
    ...
    def check_answer(self, selected_option):
        ...
        # Clear the window and create a feedback frame
        self.clear_window()

        # Set the background color based on whether the answer is correct
        feedback_color = "#cde777" if selected_index == correct_index else "#E76C6C"
        feedback_frame = tk.Frame(self.root, bg=feedback_color)
        feedback_frame.grid(row=0, column=0, columnspan=2, sticky="nsew", padx=10, pady=10)

        # Set the feedback text based on correctness
        ...
        # Next round button with reduced padding
        ...
        # Increment round count and question index
